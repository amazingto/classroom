				...
				<scroll-view scroll-y class="scoll-y">
					<view class="catalog-list" v-for="(item, i) in catalog" :key="i">
						...
						<view class="catalog-section" v-for="(items, index) in item.chaptersList" :key="index">
							<!-- <image class="img" src="../../static/mix-tree/video.png" @click="show(items.id, items.vedioaddr)"></image> -->
							...
							<!-- <image class="img1" :src="icon" @click="exercises(items.id)"></image> -->
							<button type="default" size="mini" class="btn" @click="exercises(items.id)">习题</button>
						</view>
					</view>
					<button type="primary" style="background: #999999; width: 50%;" @click="examination">点击考试</button>
				</scroll-view>
				<unik-modal ref="unikModal" :modalTitle="modalTitle" @confirmModal="confirmModal" @cancelModal="cancelModal">开始播放</unik-modal>
			</swiper-item>
		</swiper>
		<unik-modal ref="studymodal" :modalTitle="modalTitle" @confirmModal="studyconfirm" @cancelModal="cancelModal">请完成课后习题</unik-modal>
	</view>
</template>
...
export default {
	...
	onLoad(option) {
		let that = this;
		console.log(option);
		that.courseId = option.courseId;
		that.majorId = option.majorId;
		//  高度自适应
		uni.getSystemInfo({
	...
	methods: {
		...
		studyconfirm() {
			console.log('传给test的专业id：' + this.majorId);
			uni.navigateTo({
				url: '../questions/test?chapterId=' + this.chapterId + '&majorId=' + this.majorId
			});
		},
		exercisesModal: function() {
			var studentId = uni.getStorageSync('studentId');
			if (this.timer) {
				clearInterval(this.timer);
				this.timer = null;
			...
			uni.request({
				url: this.httpUrl + '/exam/operate/isComplete',
				method: 'POST',
				data: {
					chapterId: this.chapterId,
					studentId: studentId,
				...
				success: res => {
					if (res.data.code === 0) {
						this.$refs.studymodal.show();
						console.log('开始习题');
					} else if (res.data.code === -2) {
						this.tui.toast('该节的习题已经完成');
					}
				}
			});
		},
		...
		examination: function() {
			var studentId = uni.getStorageSync('studentId');
			uni.request({
				url: this.httpUrl + '/exam/operate/isExam',
				method: 'POST',
				data: {
					courseId: this.courseId,
					studentId: studentId,
				...
				success: res => {
					if (res.data.code === 0) {
						uni.navigateTo({
							url: '../questions/exam?courseId=' + this.courseId + '&majorId=' + this.majorId
						});
					} else {
						this.tui.toast('请完成所有章节的学习再考试');
					}
				}
			});
		},
