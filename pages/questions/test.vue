	...
	<view>
		...
		<view id="foot-box" class="cu-bar tabbar bg-white shadow foot">
			<view class="action" @click="submit()">
				<view class="text-gray" style="font-size: 30upx;">提交</view>
			</view>
		</view>
	</view>
</template>
			...
			uni.request({
				...
				success: res => {
					if (res.data.code === 0 && this.subjectIndex + 1 === this.subjectList.length && this.Value == this.rightAnswer) {
						...
						setTimeout(function() {
							uni.redirectTo({
								url: '../course/detail?courseId=' + courseId + '&majorId=' + majorId
							});
						}, 1000);
					} else if (this.subjectIndex + 1 != this.subjectList.length) {
